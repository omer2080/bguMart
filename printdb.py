...
def main():
    ...
    for row in table:
        ...
        print(row.name.decode(), row.salary, row.working_location.decode(), row.total_sales_income)  
    ...
    if length > 0:
        for row in table:
            ...
            x = (row.date_of_activity.decode(), row.description.decode(), row.quantity, row.name_of_seller, row.name_of_supplier)
            print(x)
